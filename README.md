...
| 서브커맨드 | 설명 |
|--------|------|
| `tfx init` | `terraform init` (backend/module 초기화, 인자 패스스루) |
| `tfx validate` | `terraform validate` (인자 패스스루) |
| `tfx fmt` | `terraform fmt` (인자 패스스루) |
| `tfx plan` | `terraform plan -out=tfplan` (인자 패스스루, AWS 계정 배너 표시) |
| `tfx sum` | plan 요약 (tree / stree / draw / md / json) |
| `tfx session [분] [-d]` | 세션 시작 — 계정 확인(뒷 4자리 입력) 후 N분간 유효. 기본은 apply만, `-d`/`--destroy` 지정 시 destroy도 허용 |
| `tfx apply` | 세션 유효 + 계정 일치 + y/n 확인 후 `terraform apply tfplan` |
| `tfx destroy` | destroy 세션에서만: destroy plan 생성 + y/n 확인 후 `terraform apply tfdestroyplan` (`-auto-approve` 거부) |
| `tfx state` | state fzf 탐색 (preview: `state show`) + list / show / mv / rm |

```bash
bb tfx init          # 최초 1회 또는 backend/module 변경 후
bb tfx validate
bb tfx fmt -recursive
bb tfx plan && bb tfx sum tree
bb tfx sum md plan-summary.md
bb tfx session 15    # 현재 AWS 계정 확인 → 15분 apply 세션 시작
bb tfx apply         # 세션 유효 + 계정 일치 + y/n 확인 후 tfplan apply
bb tfx session 15 -d # destroy까지 허용하는 세션 시작
bb tfx destroy       # (destroy 세션에서만) destroy plan 생성 + y/n 확인 후 tfdestroyplan apply
bb tfx state         # fzf 탐색 → 선택 주소 출력 (조합용: terraform taint "$(bb tfx state)")
bb tfx state rm      # 다중 선택 → 확인 후 state에서 제거 (인프라는 유지)
```

plan 파일 apply는 terraform의 yes 확인이 생략되므로 tfx가 별도 y/n 확인을 추가한다.
destroy plan 파일은 기본 `./tfdestroyplan`에 저장하며 `TFDESTROY_PLAN_FILE`로 변경할 수 있다.
세션은 시작 시점의 AWS 계정(STS 기준)에 묶이며, apply 시점에 계정이 다르면 거부한다.
destroy는 `tfx session -d`로 시작한 destroy 세션에서만 허용되고, 일반 세션은 apply만 가능하다.
`bb setup`으로 로드되는 shell init은 raw `terraform apply`/`terraform destroy`를 기본 차단하고
`bb tfx session && bb tfx apply|destroy` 사용을 안내한다. 정말 직접 실행해야 하면
`BINBOX_ALLOW_RAW_TERRAFORM=1 terraform apply`처럼 우회하거나 `BINBOX_TERRAFORM_GUARD=0`으로 guard를 끈다.
