...
setup() {
  ...
  "$BINBOX_DIR/sec" init >/dev/null 2>&1
}
...
@test "sec: unknown command errors" {
  run "$BINBOX_DIR/sec" bogus
  [ "$status" -eq 1 ]
  [[ "$output" == *"알 수 없는 명령"* ]]
}

@test "sec init: refuses to overwrite existing key" {
  run "$BINBOX_DIR/sec" init
  [ "$status" -eq 1 ]
  [[ "$output" == *"이미"* ]]
}
...
@test "sec set/get: round-trips a value via stdin" {
  printf 'hunter2' | "$BINBOX_DIR/sec" set mydb password
  run "$BINBOX_DIR/sec" get mydb password
  [ "$status" -eq 0 ]
  [ "$output" = "hunter2" ]
}

@test "sec get: single-field service works without field name" {
  printf 'tok123' | "$BINBOX_DIR/sec" set gh token
  run "$BINBOX_DIR/sec" get gh
  [ "$status" -eq 0 ]
  [ "$output" = "tok123" ]
}

@test "sec get: multi-field service without field lists fields and errors" {
  printf 'a' | "$BINBOX_DIR/sec" set mydb user
  printf 'b' | "$BINBOX_DIR/sec" set mydb password
  run "$BINBOX_DIR/sec" get mydb
  [ "$status" -eq 1 ]
  [[ "$output" == *"필드를 지정하세요"* ]]
  [[ "$output" == *"user"* ]]
  [[ "$output" == *"password"* ]]
...
@test "sec get: missing service errors" {
  run "$BINBOX_DIR/sec" get nope
  [ "$status" -eq 1 ]
  [[ "$output" == *"존재하지 않는 service"* ]]
}

@test "sec get: missing field errors" {
  printf 'v' | "$BINBOX_DIR/sec" set mydb user
  run "$BINBOX_DIR/sec" get mydb nofield
  [ "$status" -eq 1 ]
...
@test "sec env: prints export lines with sanitized names" {
  printf 'hunter2' | "$BINBOX_DIR/sec" set my-db api.key
  run "$BINBOX_DIR/sec" env my-db
  [ "$status" -eq 0 ]
  [[ "$output" == *"export MY_DB_API_KEY=hunter2"* ]]
}

@test "sec set: rejects empty value" {
  run bash -c "printf '' | '$BINBOX_DIR/sec' set mydb password"
  [ "$status" -eq 1 ]
  [[ "$output" == *"빈 값"* ]]
}

@test "sec set: rejects invalid names" {
  run bash -c "printf 'v' | '$BINBOX_DIR/sec' set 'bad name' field"
  [ "$status" -eq 1 ]
  [[ "$output" == *"허용"* ]]
}

@test "sec set: multiline value survives round-trip" {
  printf 'line1\nline2' | "$BINBOX_DIR/sec" set svc key
  run "$BINBOX_DIR/sec" get svc key
  [ "$status" -eq 0 ]
  [ "${lines[0]}" = "line1" ]
  [ "${lines[1]}" = "line2" ]
}
...
@test "sec rm: deletes a field after confirm" {
  printf 'a' | "$BINBOX_DIR/sec" set mydb user
  printf 'b' | "$BINBOX_DIR/sec" set mydb password
  run bash -c "printf 'y' | '$BINBOX_DIR/sec' rm mydb user"
  [ "$status" -eq 0 ]
  run "$BINBOX_DIR/sec" get mydb user
  [ "$status" -eq 1 ]
}

@test "sec rm: removing last field removes the service" {
  printf 'tok' | "$BINBOX_DIR/sec" set gh token
  run bash -c "printf 'y' | '$BINBOX_DIR/sec' rm gh token"
  [ "$status" -eq 0 ]
  run "$BINBOX_DIR/sec" list
  [[ "$output" != *"gh"* ]]
}
...
@test "sec rm: n answer keeps the value" {
  printf 'tok' | "$BINBOX_DIR/sec" set gh token
  run bash -c "printf 'n' | '$BINBOX_DIR/sec' rm gh token"
  [ "$status" -eq 0 ]
  run "$BINBOX_DIR/sec" get gh token
  [ "$output" = "tok" ]
}
...
@test "sec copy: pipes value to clipboard command without stdout" {
  ...
  run bash -c "'$BINBOX_DIR/sec' copy mydb password 2>/dev/null"
  [ "$status" -eq 0 ]
  [ -z "$output" ]
...
@test "sec: wrong key fails decryption with clear error" {
  ...
  run env BINBOX_AGE_KEY="$STUB_DIR/other.key" "$BINBOX_DIR/sec" get mydb user
  [ "$status" -eq 1 ]
  [[ "$output" == *"복호화 실패"* ]]
}

@test "sec: missing key with existing store hints at backup" {
  run env BINBOX_AGE_KEY="$STUB_DIR/no-such-key" "$BINBOX_DIR/sec" list
  [ "$status" -eq 1 ]
...
@test "sec edit: invalid JSON is rejected and store stays intact" {
  ...
  run bash -c "printf 'n' | EDITOR='$STUB_DIR/break-editor' '$BINBOX_DIR/sec' edit"
  [ "$status" -eq 1 ]
  run "$BINBOX_DIR/sec" get mydb password
  [ "$output" = "hunter2" ]
}
